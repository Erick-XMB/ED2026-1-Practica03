...
-- aqui recorremos la lista, y si el elemento x esta en yaVistos se quita, sino se mantiene y se sigue revisando
...
interpretacion (Not p) i = not (interpretacion p i) -- usamos not para "negar" el valor de la formula
...
-- llamamos a la funcion de chacarEstados para ver que "Syss p q" es verdadera en todos los estados
    where    
        checarEstados [] = True --si ya no hay estados que revisar entonces se va a deolver true
        checarEstados (i:is) = if (interpretacion (Syss p q) i) then (checarEstados is) else (False)
        -- checamos que "interpretacion (Syss p q) i" sea verdad, si si es se sigue revisando, y si no devolvemos false
